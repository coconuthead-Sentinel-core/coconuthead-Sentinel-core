...
class QuantumNexusForge:
    ...
    def process_input(self, user_input: str, context: Dict = None) -> Dict[str, Any]:
        ...
        # Extract intent and concepts (for backward compatibility with demo.py)
        intent = self._extract_intent(user_input)
        concepts = self._extract_concepts(user_input, cognitive_result)
    ...
    def _extract_intent(self, user_input: str) -> Dict[str, Any]:
        ...
        # Check for information requests first (more specific)
        if any(phrase in input_lower for phrase in ["tell me about", "explain", "how does", "what is"]):
            return {"type": "information_request", "confidence": 0.90}
        elif any(word in input_lower for word in ["cognitive", "architecture", "process"]) and not any(word in input_lower for word in ["hello", "hi"]):
            return {"type": "information_request", "confidence": 0.88}
        elif any(word in input_lower for word in ["status", "system", "health"]):
            return {"type": "status_query", "confidence": 0.92}
        elif any(word in input_lower for word in ["hello", "hi", "greetings"]):
            return {"type": "greeting", "confidence": 0.95}
        else:
            return {"type": "general_query", "confidence": 0.75}
    
    def _extract_concepts(self, user_input: str, cognitive_result: Dict) -> List[str]:
        ...
        concept_keywords = {
            ...
            "processing": ["process", "processing", "computation"],
            "memory": ["memory", "storage", "retention"],
            "neurodivergent": ["neurodivergent", "adhd", "autism", "dyslexia"]
        }
        ...
        return concepts if concepts else ["general_query"]
    ...
    def _generate_enhanced_response(self, user_input: str, cognitive_result: Dict, geometry_harmony: Dict) -> str:
        ...
        elif "cognitive" in user_input.lower() and "architecture" in user_input.lower():
            base_response = "The cognitive architecture uses neurodivergent-optimized processing patterns including ADHD, autism, dyslexia, and dyscalculia modes, combined with triadic consensus and sacred geometry harmonization."
        else:
...
if __name__ == "__main__":
    # Initialize the Quantum Nexus Forge
    ...
    for i, test_input in enumerate(test_inputs, 1):
        ...
        result = nexus.process_input(test_input)
